...
<div class="max-w-7xl mx-auto">
    ...
    @if(session('message'))
        @php
            ...
            $bgClass = $isSuccess ? 'bg-green-100 border-green-400 text-green-700' : 'bg-yellow-100 border-yellow-400 text-yellow-700';
        @endphp
    ...
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end gap-4 mb-6">
            ...
            {{-- Form Filter Utama --}}
            <form method="GET" action="{{ route('absensi.manual.index') }}" class="flex flex-wrap items-end gap-3 w-full md:w-auto">
                ...
                <div>
                    ...
                    <select name="kelas" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 bg-white">
                ...
                <button class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-md transition shadow-sm">
                    🔍 Tampilkan
                </button>
            </form>

            {{-- Form Absen Masal --}}
            <form method="POST" action="{{ route('absensi.manual.storeAll') }}" onsubmit="return confirm('Yakin tandai semua {{ strtoupper($kategori) }} hadir?');">
                @csrf
                <input type="hidden" name="tanggal" value="{{ $tanggal }}">
                <input type="hidden" name="kategori" value="{{ $kategori }}"> {{-- Kirim Kategori --}}
                <input type="hidden" name="kelas_filter" value="{{ $kelasId }}">
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-md transition shadow-sm flex items-center gap-2">
        ...
        <div class="overflow-x-auto border rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($dataList as $item)
                        ...
                        <tr class="{{ $rowClass }} transition">
                            <form method="POST" action="{{ route('absensi.manual.storeManual') }}">
                                ...
                                <input type="hidden" name="id_peserta" value="{{ $item->id }}"> {{-- ID Universal --}}
                                <input type="hidden" name="tanggal" value="{{ $tanggal }}">
                                <input type="hidden" name="kelas_filter" value="{{ $kelasId }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <select name="status" class="block w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-1 bg-white">
                                        <option value="" disabled {{ $status == '' ? 'selected' : '' }}>-</option>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $kategori == 'siswa' ? '6' : '6' }}" class="px-6 py-10 text-center text-gray-500">
