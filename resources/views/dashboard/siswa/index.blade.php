    ...
    <header class="bg-gradient-to-r from-green-600 to-teal-600 text-white shadow-md sticky top-0 z-50">
        <div class="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <form action="{{ route('logout') }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white rounded px-3 py-1.5 text-xs font-medium transition duration-200">
    ...
    <main class="flex-grow container mx-auto px-4 py-6">
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg mb-6 border border-gray-200">
            <div class="flex flex-col sm:flex-row items-center gap-4">
                <div class="w-16 h-16 rounded-full bg-gradient-to-br from-teal-400 to-green-500 flex items-center justify-center text-white text-2xl font-bold flex-shrink-0 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            {{-- QR CODE SECTION --}}
            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex flex-col items-center justify-center text-center">
                ...
                <div class="p-2 border rounded-lg bg-gray-50 mb-3">
                    <img src="{{ $qrUrl }}?t={{ time() }}" alt="QR Code" class="w-36 h-36 object-contain">
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b bg-gray-50">
                ...
                <p class="text-sm text-gray-500 mb-4 bg-yellow-50 p-2 rounded border border-yellow-200">
                    ...
                    Jika tidak melengkapi <strong>Absen Pulang</strong>, maka terhitung <span class="text-red-600 font-bold">Alpha</span>.
                </p>
                ...
                <form id="filterForm" method="GET" class="flex flex-col sm:flex-row gap-3">
                    <div class="w-full sm:w-auto">
                        ...
                        <select name="bulan" id="bulan" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-1.5 px-3 border">
                    ...
                    <div class="w-full sm:w-auto">
                        ...
                        <select name="tahun" id="tahun" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-1.5 px-3 border">
                    ...
                    <div class="flex items-end">
                         <button type="submit" class="bg-green-600 text-white px-4 py-1.5 rounded text-sm hover:bg-green-700">Filter</button>
                    </div>
                </form>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @php
                            ...
                            $statusMap = [
                                'H' => ['label' => 'Hadir', 'class' => 'text-green-600 bg-green-100', 'icon' => 'fa-check-circle'],
                                'S' => ['label' => 'Sakit', 'class' => 'text-yellow-600 bg-yellow-100', 'icon' => 'fa-notes-medical'],
                                'I' => ['label' => 'Izin',  'class' => 'text-blue-600 bg-blue-100',   'icon' => 'fa-info-circle'],
                                'A' => ['label' => 'Alpa',  'class' => 'text-red-600 bg-red-100',     'icon' => 'fa-times-circle'],
                            ];
                        @endphp

                        @forelse($absensiList as $index => $row)
                            @php
                                $status = $statusMap[$row->status] ?? ['label' => $row->status, 'class' => 'text-gray-600 bg-gray-100', 'icon' => 'fa-question'];
                            @endphp
